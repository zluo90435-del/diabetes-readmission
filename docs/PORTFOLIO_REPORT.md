...
系統以 MS SQL Server 儲存之臨床資料為基礎，經特徵擴充與多模型比較後，以 **LightGBM** 作為最佳模型，預測患者是否可能於出院後 30 天內再次入院。前端以 Streamlit 提供表單輸入、風險儀表板、分級建議與 SHAP 解釋，使非工程背景的使用者也能理解預測結果。

> **一句話描述（申請表/履歷可用）**  
> 建置糖尿病再入院風險預測系統，整合 SQL 臨床資料、LightGBM 建模、SHAP 可解釋性分析與 Streamlit 互動介面；透過特徵擴充與模型比較，將 ROC-AUC 由 0.57 提升至 0.68。
...
- 目前預測力有限（ROC-AUC ≈ 0.58），**不應作為唯一決策依據**
- 特徵集精簡，未涵蓋完整臨床資訊
...
> **系統（45 秒）**  
> 前端用 Streamlit，使用者輸入臨床指標後，可看到風險機率儀表板、分級建議，以及 SHAP 解釋本次預測的關鍵因子。這讓預測不只是數字，而是可以被討論的輔助資訊。
...
建置糖尿病再入院風險預測系統，整合 SQL Server 臨床資料、LightGBM/XGBoost 模型比較、SHAP 可解釋性分析與 Streamlit 互動介面。透過特徵擴充（7→20 項）與模型迭代，ROC-AUC 由 0.57 提升至 0.68，再入院 recall 由 0.22 提升至 0.58。系統定位為出院個案管理之輔助參考工具，非診斷用途。
